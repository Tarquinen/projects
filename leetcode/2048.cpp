...
private:
  ...
  bool surroundingCellsMatch(int row, int col) {
    ...
    if (row > 0 && boardGrid[row - 1][col] == currentCell)
      return true;
    if (row < 3 && boardGrid[row + 1][col] == currentCell)
      return true;
    if (col > 0 && boardGrid[row][col - 1] == currentCell)
      return true;
    if (col < 3 && boardGrid[row][col + 1] == currentCell)
      return true;
    return false;
  }
...
public:
  ...
  int getCellValue(int row, int col) { return boardGrid[row][col]; }
  ...
  bool pushLeft() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      for (int j = 1; j < 4; j++) {
        int currentCell = boardGrid[i][j];
        if (currentCell != 0) {
          if (boardGrid[i][j - 1] == currentCell && !modified[i][j] &&
              !modified[i][j - 1]) {
            gridMoved = true;
  ...
  bool pushUp() {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      for (int i = 1; i < 4; i++) {
        int currentCell = boardGrid[i][j];
        if (currentCell != 0) {
          if (boardGrid[i - 1][j] == currentCell && !modified[i][j] &&
              !modified[i - 1][j]) {
            gridMoved = true;
