...
int *shuffledArray(int length, int minValue, int maxValue) {
  assert(
      maxValue - minValue + 1 >= length &&
      "Range is too small for the requested array length without duplicates.");
  ...
  for (int prisoner = 0; prisoner < 100; prisoner++) {
    ...
    for (int attempt = 0; attempt < 50; attempt++) {
      if (drawers[drawerToCheck] == prisoner + 1) {
        freedPrisoners++;
        // cout << " attempt: " << attempt + 1 << " checking drawer " <<
        // drawerToCheck << " for prisoner " << prisoner << " found " <<
        // drawers[drawerToCheck] << "\n";
        break;
      } else {
        // cout << " attempt: " << attempt  + 1 << " checking drawer " <<
        // drawerToCheck << " for prisoner " << prisoner << " found " <<
...
int main() {
  ...
  cout << successfulSimulations << "/" << simulations
       << " successful simulations, took " << duration << " ms\n";
