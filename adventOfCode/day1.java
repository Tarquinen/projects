    ...
    public static void part2(BufferedReader file) throws Exception {
        ...
        for (int rows = 0; rows < fileList.size(); rows++) { // loop through each row
            for (int chars = 0; chars < fileList.get(rows).length(); chars++) { // loop through each char in the string
                ...
                for (int dictIndex = 0; dictIndex < dict.length; dictIndex++) { // check the char against dict, if it
                                                                                // matches check more following chars
                    if (wordFound)
                        break;
                    int wordLength = dict[dictIndex].length();
                    int matchCounter = 0;
                    for (int extraLetters = 0; extraLetters < wordLength; extraLetters++) {
                        if (chars + extraLetters >= fileList.get(rows).length()) // stop checking for matches when you
                                                                                 // get to the end of a row
                            break;
                        if (fileList.get(rows).charAt(chars + extraLetters) != dict[dictIndex].charAt(extraLetters)) {// letter
                                                                                                                      // doesn't
                                                                                                                      // match
                                                                                                                      // so
                                                                                                                      // check
                                                                                                                      ...
                                                                                                                      // item
                            break;
                        } else if (fileList.get(rows).charAt(chars + extraLetters) == dict[dictIndex]
                                .charAt(extraLetters)) {// letters match a dictionary element
                            matchCounter++;
                            if (matchCounter == wordLength) {// word matches
                                ...
                                chars += wordLength - 2; // this is really wordLength - 1 because each iteration is +1
                                                         // as well
                                break;
                            }
                        } else {
